...
class ProductController extends Controller
{
    ...
    /**end of To get the colors */

    /**To get the sizes */
        //ajax to get product details
        public function get_product_size_details(Request $request){
            // dd($request->all());
            ...
            /**get the sizes with available stocks */
                ...
                $sizesWithStock = ProductVariation::select('size_id',DB::raw('SUM(stock_quantity) as total_stock'))
                    ->where('stock_quantity','>',0)
                    ->where('product_id',$product->id)
                    ->where('color_id',$request->color_selected)
                    ->groupBy('size_id')
                    ->get();
            ...
            return response()->json([
                "status" => true,
                "product_id" => $product->id,
                "success" => view('front._product_sizes',[ 
                    "product" => $product,
                    // "product_colors" => $resultColors,
                    "product_sizes" => $resultSizes,
    ...
    /**end of To get the sizes */


    /**To get the total stock */
        //ajax to get product details
        public function get_color_size_product_stock(Request $request){
            // dd($request->all());
            ...
            /**get the  available stocks */
                ...
                //
                $total_stock = ProductVariation::where('product_id',$product->id)
                    ->where('product_id',$product->id) 
                    ->where('color_id',$request->color_selected) 
                    ->where('size_id',$request->size_selected) 
                    ->first();
                ...
                //dd($resultColors);
            /**end of get the  available stocks */
